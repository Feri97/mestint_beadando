...
namespace mestint_beadando
{
    ...
    class BuvosKeresztAllapot : AbsztraktÁllapot
    {
        // 15.feladat
        ...
        {
            return  tabla[1, 0] + tabla[1, 1] + tabla[1, 2] + tabla[1, 3] == 26 &&
                    tabla[2, 0] + tabla[2, 1] + tabla[2, 2] + tabla[2, 3] == 26 &&
                    tabla[0, 1] + tabla[1, 1] + tabla[2, 1] + tabla[3, 1] == 26 &&
                    tabla[0, 2] + tabla[1, 2] + tabla[2, 2] + tabla[3, 2] == 26 &&
                    tabla[0, 1] + tabla[0, 2] + tabla[3, 1] + tabla[3, 2] == 26 &&
                    tabla[1, 0] + tabla[1, 3] + tabla[2, 0] + tabla[2, 3] == 26 &&
                    tabla[1, 1] + tabla[1, 2] + tabla[2, 1] + tabla[2, 2] == 26;
        }

        public BuvosKeresztAllapot()
        {
            tabla[0, 0] = -1;
            tabla[0, 3] = -1;
            tabla[3, 0] = -1;
        ...
        {
            //egy operátor minden lehetséges számra
            switch (i)
            {
                case 0: return kitölt(1);
                case 1: return kitölt(2);
                case 2: return kitölt(3);
                case 3: return kitölt(4);
                case 4: return kitölt(5);
                case 5: return kitölt(6);
                case 6: return kitölt(7);
                case 7: return kitölt(8);
                case 8: return kitölt(9);
                case 9: return kitölt(10);
                case 10: return kitölt(11);
                case 11: return kitölt(12);
            }
            return false;
        }
        public bool kitölt(int szam)
        {
            // a kapott számot beírjuk a következő üres cellába
            if (!preKitolt(szam))
            {
                return false;
            }

            BuvosKeresztAllapot ujAllapot = new BuvosKeresztAllapot();
            ...
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    if (tabla[i, j] == 0)
                    {
                        ...
                        this.y = j;
                        this.tabla[x, y] = szam;
                        ujAllapot.tabla = (int[,])this.tabla.Clone();
        ...
        private bool preKitolt(int szam)
        {
            //ellenőrizzük a mátrix tartalmazza-e a számot
            if (Contains(szam)) return false;
            else return true;
        }
        private bool Contains(int szam)
        {
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    if (this.tabla[i, j] == szam)
                    {
                        //Console.WriteLine("Van már ilyen szám");
                        return true;
                    }
                }
            }
            //Console.WriteLine("uj szam");
            return false;
        }
        ...
        {
            BuvosKeresztAllapot myClone = new BuvosKeresztAllapot();
            myClone.x = this.x;
            myClone.y = this.y;
        ...
        {
            BuvosKeresztAllapot masik = (BuvosKeresztAllapot)a;
